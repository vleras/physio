...
export default function EditProduct() {
  ...
  const { data: productData, isLoading: loading } = useAdminProduct(productId);
  ...
  const [imageDeleteConfirmation, setImageDeleteConfirmation] = useState<{
    show: boolean; imageIndex: number | null;
  ...
  useEffect(() => {
    if (productData && !initialized) {
      ...
      setTranslations(productData.translations as Record<string, TranslationData>);
      setInitialized(true);
    }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateMutation.mutateAsync({ id: productId, product: { price, images, translations } });
      toast.success("Produkti u përditësua me sukses!");
      router.push("/admin");
    } catch (error: any) {
      toast.error("Gabim në përditësimin e produktit: " + error.message);
    }
  };
  ...
  const handleImageUpload = async (files: FileList | File[]) => {
    ...
    try {
      ...
      toast.success(`${urls.length} imazh(e) u ngarkuan me sukses!`);
      setUploadProgress("");
    } catch (error: any) {
      console.error("Error uploading images:", error);
      toast.error("Gabim në ngarkimin e imazheve: " + (error.message || "Unknown error"));
    } finally {
      ...
      <main className="main-content">
        <div className="container" style={{ padding: "2rem", maxWidth: "1000px", margin: "0 auto" }}>
          <div className="modern-loader">
            ...
            <span className="modern-loader-text">Duke ngarkuar produktin...</span>
          </div>
        </div>
      </main>
    );
  ...
  if (!loading && !productData) {
    return (
      <main className="main-content">
        <div className="container" style={{ padding: "2rem", maxWidth: "1000px", margin: "0 auto" }}>
          <p className="text-center py-8 text-muted-foreground">Produkti nuk u gjet.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="main-content">
      <div className="container" style={{ padding: "2rem", maxWidth: "1000px", margin: "0 auto" }}>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Ndrysho Produktin</CardTitle>
          </CardHeader>
          <CardContent>
